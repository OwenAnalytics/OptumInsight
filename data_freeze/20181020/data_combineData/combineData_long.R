...
#' Created: 09/16/2018
#' 
#' Revisions: 10/21/2018 - Separate diagnoses and proc_cd into
#'   different data sets. The duplicacy makes the two information
#'   incompatible. 
##################################################
...
diag.vars <- c("patid", "conf_id", paste0("diag", 1:25), 
               "fst_dt", "lst_dt", "copay_sum", "proc_cd", "pos")
medical_diag_long <- melt(test[, (diag.vars), with = FALSE],
...
medical_diag <- dcast(setDT(medical_diag_long),
...
medical.inpatient <- unique(medical[conf_id != "",])
