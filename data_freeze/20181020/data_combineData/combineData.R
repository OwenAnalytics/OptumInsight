...
for (j in paste0("diag", 1:25)) 
  set(test, j = j, value = gsub("^$|-|^0{3,5}$", NA, test[[j]]))
...
test[, copay_sum := sum(copay), by = c("patid", "fst_dt", "pos")]
...
# test.medical_diag1_long <- test.medical_diag1_long[order(
...
# test.medical_diagX_long <- test.medical_diagX_long[order(
#   patid, fst_dt, lst_dt, conf_id, pos), ]
...
names_in_med_not_conf <- colnames(test.medical_all)[
  !colnames(test.medical_all) %in% colnames(confinement)]

confinement2 <- confinement[, (names_in_med_not_conf) := NA]

# re-order columns
confinement2 <- confinement2[, colnames(test.medical_all), with = FALSE]

test.med_conf <- rbind(test.medical_all, confinement2)

test.med_conf <- test.med_conf[order(patid, fst_dt, lst_dt, source), ]


# test.med_conf$pos <- as.character(test.med_conf$pos)
# add pos details
...
test.med_conf2 <- merge(x = test.med_conf,
                        y = pos_detail,
                        by = "pos", all.x = TRUE)
...
proc_med <- unique(test[, .(patid, conf_id, fst_dt, lst_dt, proc_cd)])
...
med_conf_inpatient3 <- merge(x = test.med_conf_inpatient2,
                             y = proc_med_wide,
