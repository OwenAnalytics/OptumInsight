...
#' Created: 12/15/2018
#' 
#' Revisions: 01/10/2019 - updated results on the LDA with correct counts
##################################################
...
codeList <- fread("../icd9_3digitCodeList.txt",
...
pdf("plot_LDADiagnosis.pdf", width = 30, height = 15)
for (j in 1:nModels){
  # get the model
  ...
  print(topTerms %>%
    ggplot(mapping = aes(term, beta, label = name, fill = factor(topic))) +
