...
add_p_to_AC <- function(data, ac.var, summary_table_name){
  ...
  vars_to_test <- rbind(
    ...
    c("fed_poverty", "Federal Poverty Level"),
    c("home_ownership", "Home Ownership"),
    c("income_range", "Household Income Range"),
    c("networth_range", "Networth Range"),
    c("occupation", "Occupation"),
    c("race", "Race"),
    c("division", "Region"),
    c("product", "Insurance"),
    c(paste0(gsub("2", "", ac.var), "_copay_rng"), "Copay")
  )
...
table2_ac3mo <- add_p_to_AC(data = data_ac3mo, "ac3mo2", table2_ac3mo)
table2_ac4mo <- add_p_to_AC(data = data_ac4mo, "ac4mo2", table2_ac4mo)
