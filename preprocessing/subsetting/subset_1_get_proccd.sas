/****************************************************************************
* Program: subset_1_get_proccd.sas			   
*
...
*
* Description: This program extracts procedure codes (PROC_CD) of patients
* in the member data set. The extracted procedure codes will be used in main
* subsetting program. The procedure codes include CPT/HCPCS codes of smoking,
* chemo procedures, and surgery. The desired CPT/HCPCS codes are located at 
* ~\Box Sync\OptumInsight_DataManagement\preprocessing\others\
* All_codes_ICD9_NDC_HCPCS.xlsx. They are obtained from medical and facility
* claims data.
*
...
proc sql noprint;
	...
	where proc_cd in (select code from all_codes) 
		or 10000 <= input(proc_cd, best8.0) <= 69990;
quit;
