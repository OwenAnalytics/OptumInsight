/****************************************************************************
* Program: subset_2_inclusionExclusion.sas			   
*
* Location: ~\Box Sync\OptumInsight_DataManagement\preprocessing\subsetting
*
* Author: Mengbing Li 
*
...
*
* Description: 
* This program implements all inclusion and exclusion criteria
* as described in case_definition.docx. Logic of the program is described as
* the following. Go to process_of_subsetting.pptx for more details.
* 1. Exclude patients who have no records of anticoagulant from 2007-2016.
* 2. Find the index cancer date and index cancer type: the first cancer 
* occurrence among all cancer diagnoses. A patient is diagnosed with cancer
* if one of the diagnosis codes from medical data or confinement data is 
* an ICD-9 cancer code.
* 3. Exclude patients whose cancer is not active prior to index VTE date. An
* active cancer is accompanied with at least one HCPCS code of chemo 
* procedure, or NDC code for chemo drug. HCPCS codes come from medical data.
* NDC codes come from medical data and pharm data.
* 4. Check for duplicates: keep distinct rows, particularly with distinct 
* patid and clmid.
*
* Revisions: 09/11/2018 - Revise header information 
*****************************************************************************/
...
*** SELECT PATIENTS WHOSE HCPCS CHEMO PROCEDURES WERE PRIOR TO INDEX VTE ***;
	...
	from cancerobs6 a, subsetting.subset_1_get_proccd b 
...
proc export data=final_withdup
   outfile='C:\Users\mengbing\Box Sync\OptumInsight_DataManagement\data\subset_2_inclusionExclusion_withDuplicates.xlsx'
   dbms=xlsx
   replace;
   sheet="subset_2_withDuplicates";
   putnames=yes;
run;
...
/* 
	5. MERGE ROWS WITH THE SAME AC ON THE SAME DATE BUT ONLY DIFFER IN CLMID 
*/;
proc sql noprint;
...
proc export data=final
   outfile='C:\Users\mengbing\Box Sync\OptumInsight_DataManagement\data\subset_2_inclusionExclusion_noDuplicates.xlsx'
   dbms=xlsx
   replace;
