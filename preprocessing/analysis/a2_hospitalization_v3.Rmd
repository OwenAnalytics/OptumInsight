...
```{r}
# obtain inpatient claims from medical claims data by matching conf_ID from confinement claims data
medical <- fread("C:/Users/mengbing/Box Sync/OptumInsight_DataManagement/preprocessing/data/prog7_med.txt",
                 select = c("patid", "Conf_Id", paste0("Diag", 1:25),
                            "Fst_Dt"),
                 colClasses = list(character=c("patid", "Conf_Id", paste0("Diag", 1:25), "Fst_Dt")))
colnames(medical) <- tolower(colnames(medical))

conf <- data.table(read_excel("C:/Users/mengbing/Box Sync/OptumInsight_DataManagement/preprocessing/data/confinement.xlsx"))
colnames(conf) <- tolower(colnames(conf))
# keep hospitalizations after index VTE date
