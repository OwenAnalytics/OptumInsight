...
%macro create_conf(yr=);
	select strip(put(patid,20.)) as patid, admit_date format=YYMMDD10. as admit_dt label="Admit Date", 
		...
		charge, coins, copay, deduct, los, pos, prov, conf_id
	from conf.conf_dvt_&yr(keep=patid admit_date diag1-diag5 disch_date charge coins copay deduct los pos prov conf_id)
/*	where calculated patid in (select distinct strip(patid) from patids)*/
%mend;

proc sql noprint;
*** READ IN CONFINEMENT INFORMATION OF ALL PATIENTS OVER ALL YEARS ***;
...
proc export data=confs 
	outfile='C:\Users\mengbing\Box Sync\OptumInsight_DataManagement\data\conf_all.txt' 
	dbms=tab replace;
run;


proc sql;
*** SELECT CONFINEMENT OF ELIGIBLE PATIENTS ***;
	create table confs2 as
	select * 
	from confs
	where patid in (select distinct strip(patid) from patids);

*** ADD INDEX VTE DATE ***;
	create table conf_vte as
	select a.*, b.index_dt
	from confs2 a left join patids b
	on a.patid = b.patid;
quit;
