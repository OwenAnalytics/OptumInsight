...
### Model:  
$\log{ \left( \frac{P(\text{AC4Mo = j})}{P(\text{AC3Mo = LMWH})} \right)} =$ vte\_VTENames + vte\_history + hospitalized + malignancy + charlson_comorb_score + division + education2 + income\_range + ac3mo\_copay\_range + product + age\_s + male + race, where $j$ can be "Warfarin", "DOAC", "Unknown/Multiple".
...
create_coefficients_table(fit.ac4mo, "")
```
...
summary(fit.age_cat <- glm(age_cat ~ ac3mo2, family = "poisson", data = data_ac3mo))
```
